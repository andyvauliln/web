...
import { getRandomLand } from "~/components/mocks"
...
export default function LandList() {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4">
            {[1, 2, 3, 4, 5].map((item, idex) => (
                <div key={idex} className="items-center bg-gray-50 rounded-lg shadow sm:flex dark:bg-gray-800 dark:border-gray-700 p-2">
                    <Image className='rounded-md' src="/land_sideman.gif" alt="Your GIF" width={300} height={200} />
                    <div className='border-b w-auto border-gray-200 py-2 grid grid-cols-3 gap-2'>
                        {/* <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">{getOwnershipType().name}</span>
                        <span className="bg-pink-100 text-pink-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-pink-400 border border-pink-400">{getRegion().name}</span>
                        <span className="bg-pink-100 text-pink-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-pink-400 border border-pink-400">{getLandZone().title}</span>
                        <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-4000">{getLandSize().name}</span> */}
                    </div>
                </div>
            ))}
        </div>
